...
</head> <?php include $_SERVER['DOCUMENT_ROOT'] . "/travel_and_transport/includes/connection.php"?> <?php

if ($_SERVER["REQUEST_METHOD"] == 'POST') {
    if (!empty(trim($_POST["login2username"])) && !empty(trim($_POST["login2password"]))) {
        $username = trim($_POST["login2username"]);
        $password = $_POST['login2password'];
        $category = "user";
        if (!empty($username) && !empty($password)) {
            ...
            if (!empty($row)) {
                ...
                $_SESSION["id"] = 1001;
                $_SESSION["login2username"] = $_POST["login2username"];
                $_SESSION['loggedin_time'] = time();
                if (isset($_SESSION["id"])) {
                    if (!isLoginSessionExpired()) {
                        header('location: /travel_and_transport/forms/User');
        ...
        header('location: /travel_and_transport/index.php?msg="Email or Password is Empty"');
    }
}
?>
